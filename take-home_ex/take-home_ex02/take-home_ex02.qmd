...
```{r}
temp = select(jakarta_vaccination_rate, c(2, 8, 13, 14))
...
-   KAMAL MUARA - 3172011002

-   HALIM PERDANA KUSUMA - 3175081004

-   KELAPA GADING TIMUR - 3172061001

```{r}
cbg <- gi_stars %>%
  ungroup() %>%
  filter(village_code %in% c(3172011002, 3175081004, 3172061001)) %>%
...
# km <- gi_stars %>%
#   ungroup() %>%
#   filter(village_code == 3172011002) %>%
#   select(sub_district.x, `date`, gi_star)
# 
# ggplot(data = km,
#        aes(x = `date`,
#            y = gi_star)) +
#   geom_line() +
#   theme_light()
km = filter(cbg, village_code == 3172011002)
ggplot(data = km,
       aes(x = date,
           y = gi_star)) +
  geom_line() +
...
```{r}
hpk = filter(cbg, village_code == 3175081004)
ggplot(data = hpk, 
       aes(x = `date`, 
           y = gi_star)) +
  geom_line() +
  theme_light() + 
  ggtitle("HALIM PERDANA KUSUMA")
```
...
```{r}
kgt = filter(cbg, village_code == 3172061001)
ggplot(data = kgt, 
       aes(x = `date`, 
           y = gi_star)) +
  geom_line() +
  theme_light() + 
  ggtitle("KELAPA GADING TIMUR")
```
...
-   **KAMAL MUARA**: We see a very sharp upwards trend in the first 4 months and a sharp decline after Oct 2021. Since the tau value is negative and closer to -1 we can see that there is a strong decreasing trend. The p-value is also less than 0.05 which indicates that the trend is statistically significant and we can reject the null hypothesis that there is no trend. This trend matches our previous observations in Section 6.3 and Section 7.3.

-   **HALIM PERDANA KUSUMA**: We see a very irregular pattern in the trend. The tau value is positive but not too close to 1 which shows a weak upwards trend. However, since the p-value is above 0.05 we cannot reject the null hypothesis that there is no monotonic trend in the series. This is interesting as our conclusion from Section 6.3 was that this sub-district is smaller and thus is more likely to have a higher vaccination rate even if vaccination efforts are reduced over time. This "trend" shows us this irregularity.

-   **KELAPA GADING TIMUR**: We see a strong downward trend and the tau value is negative and close to -1. Since the p-value is \< 0.05 the trend is statistically significant and we can reject the null hypothesis. This chart further supports our analysis done in Section 6.3 that their vaccination rates seem to drop over time.
...
According to the above code chunk we can see that these are the significant emerging hot/cold spots:

+--------------+-------------------+-----------------------------------------------------------+
| Village Code | Sub-District Name | Hot/Cold Spot                                             |
+==============+===================+===========================================================+
| 3171011003   | PETOJO UTARA      | Cold Spot - there is a significant strong downward trend. |
+--------------+-------------------+-----------------------------------------------------------+
| 3175011003   | KAYU MANIS        | Hot Spot - there is a significant strong upward trend.    |
+--------------+-------------------+-----------------------------------------------------------+
| 3175011001   | PISANGAN BARU     | Hot Spot - there is a significant strong upward trend.    |
+--------------+-------------------+-----------------------------------------------------------+
| 3175021004   | JATINEGARA KAUM   | Hot Spot - there is a significant strong upward trend.    |
+--------------+-------------------+-----------------------------------------------------------+
| 3171021001   | PASAR BARU        | Cold Spot - there is a significant strong downward trend. |
+--------------+-------------------+-----------------------------------------------------------+

: The emerging hot/cold spots in DKI Jakarta
